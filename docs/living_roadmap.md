...
- **Evaluations**: 360x dev-mini caption run completed (100 clips) vs. TAL refs; summary at `data/eval/captioning/360x_devmini/summary.json` shows BLEU ≈ 0.0037, CIDEr ≈ 0.0059, SPICE ≈ 0.0499. QA manifest + refs exist but baseline metrics are still pending.
- **Docs & tooling**: Vast setup script now installs `hf_transfer`, skips existing datasets, and renders a single aggregate HF download progress bar. README was trimmed and now spotlights the Vast workflow plus quick commands.
- **Open gaps**: Need refreshed caption metrics after prompt tuning, QA baseline metrics, and a longer-form docs surface (GitHub Pages or MkDocs) for architecture details.

---

## Priority Stack
1. **360x Captioning Refresh (High)**  
   - Command: `uv run python -m captionqa.captioning.baseline --manifest data/eval/captioning/360x_devmini/manifest.jsonl --engine qwen_vl --output-dir data/eval/captioning/360x_devmini --refs data/eval/captioning/360x_devmini/refs.jsonl`  
   - Deliverable: Updated predictions/metrics after prompt/config tweaks; log in README + this doc.  
   - Owner: Will (prefer Vast A10 with ≥80 GB storage).

2. **QA Baseline (High)**  
   - Use TAL QA manifest `data/eval/qa/360x_devmini/manifest.jsonl` and run `captionqa.qa.baseline_vqa`.  
   - Capture accuracy/F1 in `data/eval/qa/360x_devmini/summary.json` and surface results in README.
...
4. **Prompt & Config Iterations (Medium)**  
   - Sweep Qwen-VL prompts, frame counts, and temporal windows.  
   - Use reduced manifests (`--limit 20`) for fast loops before running full 100-clip sets.
...
## Near-Term Roadmap
### 0-2 Days
- ✅ Vast setup script upgrades (hf_transfer install, skip existing datasets, aggregate progress bar) – 2025-11-15.  
- ✅ README trimmed + Vast workflow documented – 2025-11-15.  
- ⏳ Re-run captioning baseline with refreshed prompts and log metrics.  
- ⏳ Kick off first QA baseline run and archive metrics + commands.  
...
- ⏳ Validate Vast workflow with persistent storage or larger disks; capture the recommendation in README/docs.  
