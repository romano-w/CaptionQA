...
- **Prompt/decoding sweeps (captioning)**: Added an action-heavy caption template plus `--qwen-temperature/--qwen-top-p/--qwen-max-new-tokens` controls to `captionqa.captioning.baseline` to mirror the QA runner. GPU access was unavailable in this environment, so the A/B runs over 20–30 clips are queued; once Qwen-VL can execute, run the sweep with `--action-heavy-prompt --qwen-temperature 0.2 --qwen-top-p 0.9 --qwen-max-new-tokens 160` and log BLEU/CIDEr deltas.
