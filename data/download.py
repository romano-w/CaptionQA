"""Utilities for downloading the datasets referenced in the project README.

The module exposes a simple command line interface that can download each
dataset to a dedicated directory under a user provided output path.  Each
dataset is represented by a :class:`DatasetTask` that knows how to fetch its
artifacts (archives, git repositories, or Hugging Face snapshots).

Example usage::

    python -m data.download --list
    python -m data.download 360x --output ./datasets
    python -m data.download leader360v --output ./datasets --overwrite

Because some datasets are large, the download functions stream the payload to
disk and show a progress bar when the server exposes a content length.  Archive
files (``.zip`` / ``.tar[.gz|.bz2]``) are extracted automatically when the
dataset task specifies an extraction directory.

The actual dataset hosting endpoints occasionally change.  If a download fails
with a 404 the caller can supply an updated URL by editing the
``DATASETS`` definition below.  The command line interface intentionally
surfaces any HTTP or subprocess errors so that issues are visible to the user.
"""
...
class DownloadArtifact:
    """Definition for a single downloadable artifact.
    ...
    filename:
        Optional file name override.  If omitted the name is derived from the
        URL's path component.
    extract:
        When ``True`` the downloaded archive is extracted into the dataset
        directory.
    extract_subdir:
        Optional sub-directory inside the dataset directory to receive the
        extracted content.  When omitted the archive is extracted directly into
        the dataset root.
    checksum:
        Optional hexadecimal SHA256 checksum for verifying the download.
    """
...
@dataclass(frozen=True)
class DatasetTask:
    """Task metadata for a dataset download."""
...
def download_360x(dataset_dir: Path, overwrite: bool, dry_run: bool) -> None:
    """Download the 360+x dataset snapshots from Hugging Face."""
...
def download_360dvd(dataset_dir: Path, overwrite: bool, dry_run: bool) -> None:
    """Download the 360DVD dataset archive from Google Drive."""
...
def download_leader360v(dataset_dir: Path, overwrite: bool, dry_run: bool) -> None:
    """Download the Leader360V dataset snapshot from Hugging Face."""
...
def download_360sr(dataset_dir: Path, overwrite: bool, dry_run: bool) -> None:
    """Download the 360SR Challenge dataset assets from Google Drive."""
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    except Exception as exc:  # pragma: no cover - CLI surface for debugging
